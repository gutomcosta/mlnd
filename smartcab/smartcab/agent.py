...
class LearningAgent(Agent):
    ...
    def get_maxQ(self, state):
        ...
        maxQ = self.get_max_action_value(state)[1]

        return maxQ 


    def get_initial_values_for_actions(self):
        initial_values = {}
        for action in self.valid_actions:
            initial_values[action] = 0.0
    ...
    def choose_action(self, state):
        ...
        if self.learning:
            epsilon_probability = random.randint(0, 1)
            if epsilon_probability < self.epsilon:
                action = self.random_action()
            else:
                action = self.get_max_action_value(state)[0]
        else:
            action = self.random_action()
